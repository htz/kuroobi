...
### 棄却: ETC (Enhanced Transposition Cutoff)

子を展開する前に各子の置換表を引き、子の上界が −β 以下ならその節点を
即カットする手法 (Edax が持つ)。**厳密で、値も手も変えない** — 使うのは
置換表の上下限そのもので、`(depth, relax)` の門は `put`/`get` と同じ。
だから測るのは速度だけでよい。
...
Edax で ETC が効くのは、そちらの並べ替えがここまで詰まっていないため
と考えられる。**同じ手法が同じ効果を持つとは限らない。**
