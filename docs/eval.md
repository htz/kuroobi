...
> **この判定は leaf 再構成方式への変更で失効した。** 上の「遅い」は増分維持
> 経路の数字で、そこでは H=64 は確かに 3 倍近く遅い (i16 で 7.54 対 2.48
> Mnps)。だが**内部ノードの累算を撤廃した**あと、H が効く場所が無くなった。
> 本番経路 (leaf 再構成) を線形基準で 3 回ずつ交互に測ると **H=16 が 1.02x、
> H=64 が 1.10x** — **幅 4 倍の代償は 8%** しかない。律速は FT の総和ではなく
> インデックス計算と着手生成の側にある。
>
> 幅を広げる価値も当時より大きい。同じ 2026-07-27 の 3 点 (H=8 39.67 /
> H=16 36.10 / H=64 33.63) は**幅が単調に効いている**ことを示しており、
> H=64 は当時の H=16 に 2.5 勝っていた。「H=64 (33.63) より H=16 の方が良い」
> という後年の記述は、H=64 の旧値を**学習データも手順も違う**新しい H=16
> (31.05) と比べた誤りである。
