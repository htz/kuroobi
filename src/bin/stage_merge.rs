...
//!
//! Usage: stage_merge --val <file.data> --out <path> <weights.bin>...
...
fn main() -> ExitCode {
    ...
    // [count, sum_abs, sum_sq] per stage, per input.
    let mut scores: Vec<Vec<[f64; 3]>> = Vec::with_capacity(inputs.len());
    ...
    for p in &inputs {
        ...
        for ex in &val {
            let board = ex.board();
            let e = ex.score as f64 - ev.eval(&board) as f64;
    ...
    println!("  stage  empties       n      MSE     MAE   from");
    for (st, first) in scores[0].iter().enumerate().take(STAGE_COUNT) {
        let n = first[0];
        if n == 0.0 {
            continue;
        }
        let (best, mae) = (0..inputs.len())
            .map(|i| (i, scores[i][st][1] / scores[i][st][0]))
        ...
        println!(
            ...
            60 - st,
            n as u64,
            scores[best][st][2] / scores[best][st][0],
            mae,
            inputs[best].display()
        );
    }
