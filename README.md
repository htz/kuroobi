...
(Young Brothers Wait Concept)。Edax も同じ YBWC。厳密解は全スレッド数で
逐次と一致する。

1 分割点あたりのスレーブ数 (`SPLIT_MAX_SLAVES`) は **2**。Edax は 1、
Egaroucid は per-node の上限を設けていない。
FFO40-49 (474M ノード、決定論的、3 巡最小、逐次 21.48 秒):

| 上限 | 6 スレッド | 8 スレッド | 10 スレッド |
|---|---|---|---|
| 1 | 2.62x | 2.69x | 2.67x |
| **2** | 2.71x | **3.03x** | **3.16x** |
| 3 | — | 3.03x | 3.06x |

3 以上は浅い集合で崩れる (FFO40-44 の 6 スレッドで 1 -> 2.21x, 2 -> 2.28x,
3 -> 2.00x, 4 -> 2.02x)。広い 1 ノードが予算を独占して木の他の部分を飢えさせる
—— 上限 1 が安全側に見える理由がこれである。**2 が両方の集合で頑健。**

> 分割は思ったより少ない。FFO40-44 の 8 スレッドで分割 1,830 回 / 生成スレッド
> 3,044 本しかない (`solve_obf` が出力する)。**分割ごとに OS スレッドを生成して
> いても、その費用は問題にならない** (2 スレッドなら 52 回)。効いているのは
> 分割点の総数そのもので、`PARALLEL_MIN_EMPTIES = 16` が上位数手しか分割対象に
> しないため 2 スレッドでも 1.37x しか出ない。

#### Egaroucid の終盤並列度との比較

同一機械、`-noise` で `main end` 行を読む。30 空きの局面を Egaroucid の
level 21 (= 93% 選択度の終盤探索、63.8M ノード) で解かせると:

| | t=2 | t=4 | t=6 | t=8 | t=10 |
|---|---|---|---|---|---|
| 速度比 | 1.65x | 2.96x | **4.09x** | **5.02x** | 5.35x |
| ノード膨張 | 1.01 | 1.03 | 0.98 | 0.97 | 1.04 |

**膨張ゼロで 6 並列 4.09x。**当方は同規模 (FFO40-49, 474M) で 6 並列 2.71x /
膨張 1.13。ここは実装差である: 相手は終盤でも中盤と同じ並列機構
(アイドルゲート付きの単一プール、per-node 上限なし、停止フラグ連鎖) を
使うのに対し、当方の終盤ソルバは兄弟を共有
キューで取り合う別方式で、分割ごとにスレッドを生成する。次はここを中盤側の
実装に寄せる。

> 同一局面を当方の `solve_obf` (100% 完全読み) で解くと 543 秒 / 120 億ノード
> かかる。Egaroucid の 3.1 秒 / 6380 万ノードとは**探索の質が違う** (93% 選択度)
> ので、絶対速度の比較には使えない。並列度の比較にはノード規模が近い FFO を使う。
