...
FFO テスト 71 問 (fforum-1〜71、空きマス 6〜34、値は全問正解) を Edax
(公開版 4.5.5、既定設定 `-h 21`) と 1 問ずつ突き合わせた。当方は既定の
2^26 表。どちらも単一スレッド (Edax は macOS では Rosetta 経由の x86
バイナリ)。fforum-72 以降は両者とも未計測。★ は当方が Edax のノード数を
下回った問題 (52/71)。t比・NPS の「—」は Edax 側の時間が 0.005 秒未満に
丸められ算出不能。

総計はノード 0.44 倍・時間 0.80 倍で Edax を上回る。1 ノードあたりの
速度 (NPS) は当方が約半分だが、これは並べ替えの先読み評価を厚くかけて
ノード数を削っている分で、その NPS 差を吸収した上でのノード・時間の
優位である。空きマスが増えるほど差が開き、最難関の FFO60-71 は全 12 問
でノードを下回った。
...
| **計** | | | **39,234,910,733** | **88,368,093,628** | **0.44** | **6553** | **8212** | **0.80** | **6.0M** | **10.8M** |
