...
上限は**独立プロセスを 6 個走らせた合計スループット 5.4x** (共有も同期も無い
条件)。メモリ帯域律速ではない。

d16 / 20 局 / 対 Egaroucid L15、3 巡の最小値:

| 版 | 1 並列 | 6 並列 | 速度比 | 1 並列のノード |
|---|---|---|---|---|
| 修正前 | 42.5s | 25.8s | 1.65x | 261M |
| `searchings` 連鎖 + 3-way 置換表 + 無ロック分割判定 | **31.5s** | **15.6s** | **2.12x** | 172M |

棋力は落ちていない (60 局の自己対局、d16): 6 並列 **50.0%** (27W 27L 6D, 石差
+0.02)、8 並列 48.3% (27W 29L 4D, 石差 +0.02)。
...
= 分割点そのものが足りない。深さを上げると素直に伸びる。スレッド数は d18 で
**8 が最適** (6 -> 1.71x, 8 -> **1.99x**, 11 -> 1.49x)。実コア 10 に対し 11 は
一貫して悪い。

残差の内訳 (d16, 6 並列): 使える CPU 時間の 46% しか働いておらず、しかも働いて
いるコアの nps が逐次の 66% しか出ていない (0.46 x 6 x 0.66 = 1.81x)。**主
スレッドは中盤時間の 40〜60% を「キューが空でタスクを盗めない」状態で潰して
いる**。ノード数のオーバーヘッドはもう無い (6 並列 165M 対 1 並列 172M) ので、
残りは全部この 2 つ。
...
> **置換表に lower/upper の 2 境界を持たせる**案は、
> 最初の実装で fail-low 時に並べ替え用の手を落としてしまい、ノードが 3.4 倍に
> 爆発したので取り下げた。着想自体は正しいので、手の保存を分離してから再挑戦する。
