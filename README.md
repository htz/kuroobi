...
|---|---|---|---|---|---|
| 速度比 | 1.65× | 2.96× | **4.09×** | 5.02× | 5.35× |
| ノード膨張 | 1.01 | 1.03 | 0.98 | 0.97 | 1.04 |

当方は FFO40-49 (20〜26 空き、474M ノード) で 10 スレッド 4.34×。**Egaroucid の
6 スレッド 4.09× を上回った**が、これは同じ土俵ではない: Egaroucid の
測定局面は 30 空きで、分割対象になる層が 17〜30 の 14 段ある。当方の
FFO40-49 は 20〜26 空きなので層は 4〜10 段しかない。中盤で確かめたとおり
**並列度は木の大きさで決まる**ので、段数が違えば比は違う。ノード膨張は
まだ当方が大きい (1.26 対 1.0) 一方で、上限なしの投機がその膨張を払って
なお速度を買っている、という関係になっている。
