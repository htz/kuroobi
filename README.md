...
├── nnue.rs            NNUE 評価関数 (int16 量子化 + NEON) (→ §12)
├── search.rs          中盤探索: PVS + ETC + ProbCut (→ §6)
├── midgame.rs         対局用 NNUE 中盤探索: YBWC + Lazy SMP (→ §12)
├── solver.rs          終盤完全読みソルバ + 選択読み (→ §7, §11)
├── stability.rs       確定石 (→ §5)
