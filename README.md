...
GGS (skatgame.net:5000) の 8x8 レート戦に **Kuroobi** (ログイン名 `kuroobi`) として参戦している。
8r (ランダム開局・同期ペア) プールでレート **2580±181** — 対 Rhapsody (2630)
に 5 ペア全て ±1.0 以内。クライアントは `src/bin/ggs.rs` で、時計管理と
残り時間による自動減速、切断時の自動再接続と対局の自動再開 (絶対不放棄) を
実装している。
...
- **棋譜** — 貼り付けとファイルの両方から読み込み、着手をクリックすると
  ...
  始まらない対局 (抽選オープニング) も開始局面ごと再現できる。素の着手列
  (`f5d6c3…`) と、盤面 1 行 + 着手列の形式も従来どおり受け付ける。
