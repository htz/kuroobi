...
(Young Brothers Wait Concept)。Edax も同じ YBWC で、こちらは 1 分割点
あたりのスレーブを 3 に制限している (`SPLIT_MAX_SLAVES`)。厳密解は
全スレッド数で逐次と一致する。
...
### 探索の並列化 (Lazy SMP、逐次と完全一致)

NNUE 中盤探索は Lazy SMP で並列化してある。**同じ深さなら並列度をいくら
上げても結論は変わらない**のが要件で、変わればそれは棋力低下である。

検証は勝率では行わない。200 局でも 95% 信頼区間が ±7% あり、1% 級の劣化を
見抜けないため。代わりに `lab --verify-parallel` が局面ごとに逐次と
並列の**指し手と根の評価値**を突き合わせる (評価値は f32 のビット比較)。

| 条件 | 指し手一致 | 評価値不一致 |
|---|---|---|
| depth 12 / 8 スレッド / 200 局面 | 200/200 | 0 |
| depth 10 / 4 スレッド | 100/100 | 0 |
| depth 14 / 8 スレッド | 100/100 | 0 |
| depth 12 / 16 スレッド | 100/100 | 0 |

**ProbCut を使う探索では並べ替えが結果を決める** (順序 → 窓 → どこを刈るか)
ので、helper の情報が主スレッドに一滴でも混ざると答えが変わる。一致を
壊していた 3 経路は、いずれも上の検証で見つけた:

1. **helper の評価値** — TT に選択度を記録し、より強く刈った結果は使わない。
   格付けは (深さ, 選択度) の辞書式で行う (91/100)
2. **helper の最善手** — 順序ヒントとしてだけでも結果を変える (98/100)
3. **helper の書き込みが主スレッドのエントリを追い出す** — ハッシュにソルトを
   かけて同じ配列を分けるだけでは足りない。主スレッドは値を誤読しないが、
   枠を奪われたノードを再探索して別の刈り方をする。**helper には物理的に
   別の表**を与えて解決 (200/200)
...
| 構成 | 一致 | L15 の速度 |
|---|---|---|
| helper が主スレッドに寄与 | 88〜98% (**棋力低下**) | 1.8x |
| **helper を完全隔離 (採用)** | **100%** | **1.10x** |

厳密一致を要求すると helper は主スレッドに寄与できず、Lazy SMP の価値の源泉
(helper の発見を主が再利用する) が失われる。**1.10x は helper が共有キャッシュを
温める効果のみで、これが厳密一致下での上限**である。1.8x 出る版は指し手が
88〜98% しか一致しないため採らない。

> 棄却した並列化: **ルート分割**は単一スレッドに負けた (1.7 倍のノードで 1.3 倍
> 遅い)。ルートでは最年長の子の部分木がほぼ探索全体なので分配できる仕事が
> 少なく、alpha が締まる前に走らせるぶん無駄になる。

### 学習

`nnue_train` が Hogwild 並列 SGD (10 スレッドで 2.2M pos/s) で学習する。
**データ量が支配的**: 2400 万局面では val 40.7 で過学習、1 億局面で 36.1 まで
下がった。lr 0.0005 前後、`--val` で毎エポック held-out MSE を測り best を保存。
