...
|---|---|---|
| f32 スカラー・増分維持 | 16.3x | — |
| int16 量子化 + NEON | 2.64x | FT のメモリが半分、SIMD 幅 2 倍 |
| 視点 interleave (2H 一括更新) | 1.51x | マスクあたりのロードが半減 |
| **leaf 再構成方式** | **1.12x** | **内部ノードの累算を撤廃** |
| 視点別の分離 FT | **1.05x** | leaf のロード量が半減 |
...
i16 の量子化損失は **0.05 石² (0.1%)** で無視できる一方、速度は 5 倍違う。
2 バイトなら FT がキャッシュに乗り、NEON も int16x8 で幅が 2 倍になる。
...
H=8 は線形を上回れず、速くても意味がない。H=16 が強さと速度の最適点。
