...
FFO40-49 (474M ノード、決定論的、最小値):

| スレッド | 秒 | 速度比 | ノード膨張 | うち表クリア |
|---:|---:|---:|---:|---:|
| 1 | 21.203 | 1.00× | 1.00 | 0.40 |
| 4 | 8.069 | 2.63× | 1.19 | 0.18 |
| 6 | 6.051 | **3.50×** | 1.21 | 0.15 |
| 8 | 5.287 | **4.01×** | 1.31 | 0.14 |
| 10 | 4.678 | **4.53×** | 1.26 | 0.14 |
...
|---:|---:|---:|---:|---:|
| 1 | 21.037 秒 | 1.00× | 21.203 秒 | 1.00× |
| 2 | 12.808 秒 | 1.64× | 13.342 秒 | 1.61× |
| 4 | 5.410 秒 | 3.89× | 8.069 秒 | 2.63× |
| 6 | 3.763 秒 | 5.59× | 6.051 秒 | 3.50× |
| 8 | 3.028 秒 | 6.95× | 5.287 秒 | 4.01× |
| 10 | **2.843 秒** | **7.40×** | 4.678 秒 | 4.53× |

**1 スレッドでは互角 (当方が 0.8% 遅い) なのに、10 スレッドでは 1.65 倍
離される。**絶対速度の差は全部並列効率である。

> ただしこの土俵にも 1 つ偏りが残る。Egaroucid は
> `transposition_table.init()` を計時の**外**で呼ぶので、報告される時間に
> 表クリアが入らない。当方は入っている (10 スレッドで 0.14 秒 = 3%)。
> 当方も並列クリアにしたのでかつての 0.41 秒からは縮んだが、ゼロではない。
...
> 確かなのは壁時計だけ: 1 スレッド互角、10 スレッドで 1.65 倍。
...
|---|---:|---:|---:|---:|---:|
| 当方 | **68%** (33.6/49.3) | 80.6% | 1.284× | 4.28× | 4.29× |
| Egaroucid | **88%** (27.0/30.6) | 74.7% | 1.147× | 5.73× | 5.78× |

**コア当たり効率では当方が勝っている** (80.6% 対 74.7%)。負けているのは稼働率
(1.29 倍) とノード膨張 (1.12 倍) だけで、1.29 × 1.12 × 0.93 = 1.34 倍が
上の表の一律差にあたる。**稼働率が単独で最大の梃子**である。
...
(2) を移植した。`chunks_mut` + `thread::scope` で等分するだけである
(専用表は小さいので逐次のまま)。FFO40-49、10 スレッド、3 巡最小:
...
クリアは 2.9 倍で止まる (メモリ帯域律速なので 10 倍は出ない) が、
差分 0.251 秒と wall 減 0.258 秒が一致する。**全体 5.2% 短縮。**
