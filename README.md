...
浅い問題 (FFO1-19) で 8.7 倍遅いのは探索ではなく**1 局面ごとの置換表の
全消去**による。2^26 エントリ = 1.6 GB を毎回ゼロ埋めすると 1 局面あたり
約 18 ms かかり、14 空きの問題ではそれが時間の 84% を占める。表を局面規模に
合わせれば消える (`--hash-bits 20` で FFO1-19 は 0.072 秒、ノード数は不変)。
世代印で消去を O(1) にする実装も試したが、プローブ経路が重くなって
FFO40-59 が 5% 悪化したため採用していない。
...
1 問あたり 20 マイクロ秒程度しかかかっていない。表を局面規模に合わせる
運用で解消する (上の集計を参照)。
