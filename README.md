...
FFO40-59 の全 20 問 (空きマス 20〜34、値は全問正解)。Edax は既定設定
(`-h 21`)、当方は既定の 2^26 表。どちらも単一スレッド。

| 指標 | 当方 | Edax | 比 |
|---|---|---|---|
| 総ノード | **14,218M** | 25,883M | **0.55** |
| 総時間 | **1,843 秒** | 2,133 秒 | **0.86** |
| ノードで Edax 未満 | **19 / 20 問** | | |
| 時間で Edax 未満 | 14 / 20 問 | | |

局面別で最も差が大きいのは FFO58 (0.32)、FFO52 (0.38)、FFO50 (0.43)。
唯一 Edax を上回れていないのが FFO43 (1.04) で、その内訳は厳密探索
20.7M + ウォームアップ 2.6M ——**厳密探索単体では Edax を下回っている**。

代表的な難問の推移 (この改善作業の前後):

| 問題 | 改善前 | 現在 |
|---|---|---|
| FFO#53 (空き28) | 6,282M / 763 秒 | 1,813M / 190 秒 |
| FFO#57 (空き30) | 8,088M / 1,492 秒 | 962M / 138 秒 |
