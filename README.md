...
最小値で 0 -> 2.25x, 2 -> 2.39x, 8 -> **2.43x**, 32 -> 2.20x。既定は 2
(8 スレッドでは 4 つの中で最良、6 スレッドでも最良と誤差内)。
...
| + 非分割ノードで `Arc` に触らない + キュー slack | **27.3s** | **11.4s** | **2.43x** | 172M |

**3 倍には届いていない。** 同じ機械で Egaroucid は 6 並列 3.28x を出すので、これは
まだ実装差である。

棋力は落ちていない (60 局の自己対局、d16): 6 並列 **48.3%** (28W 30L 2D, 石差
-0.05)、8 並列 53.3% (31W 27L 2D, 石差 +1.07)。いずれも 95% 信頼区間が 50% を含む。
