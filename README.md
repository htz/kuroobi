...
| 施策 (6 スレッド, 60 局の自己対局) | 棋力 | 速度 |
|---|---|---|
| 上の欠陥 2 件を修正 | 48.5% | 1.35x |
| PV ノードの年少兄弟も分割 | 48.3% | 1.42x |
| 分割前に逐次で読む子を 2 に (`YBWC_ELDER`) | **50.0%** | **1.66x** |

`YBWC_ELDER` は 1 ではなく 2 が最適だった (1 -> 1.30x, 2 -> 1.66x,
3 -> 1.63x)。当方の葉評価は NNUE で重く、無駄になる部分木 1 つの代償が大きい。

スレッド数は実コア 10 に対し **6 が最適** (3 -> 1.53x, 6 -> 1.69x, 8 -> 1.48x,
10 -> 1.46x)。残る損失はワーカー稼働率 41% と、逐次の 1.87 倍というノード数。
